...
| 侧栏 Grok Bot | `Profile` 圆槽宏切：站点头像 : Bot ≈ 1:4（`profile-grok-timing.ts`）。Bot 六桶巡演。几何来自学习仓 replica，只放本机 `public/vendor/grok-bot/`（gitignore，见 ADR-0003）；线上缺文件则锁静态头像。≠ Ask 输入条 class `.ask-grok-row` |
