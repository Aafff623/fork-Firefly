(function () {
    ...
    var config = JSON.parse(window.__fireflyMusicManagerConfigStr || '{}');
    ...
    // Map config playMode string to number（可被 localStorage 覆盖）
    if (config.playMode === 'random') state.playMode = 2;
    else if (config.playMode === 'one') state.playMode = 1;
    else state.playMode = 0;
    var storedMode = localStorage.getItem('music-player-mode');
    if (storedMode !== null) {
        var parsedMode = parseInt(storedMode, 10);
        if (parsedMode >= 0 && parsedMode <= 2) state.playMode = parsedMode;
    }
    ...
    // ── Media Session（系统媒体键 / 锁屏；无 API 则跳过）──
    function updateMediaSessionMetadata(track) {
        if (!('mediaSession' in navigator) || !track) return;
        try {
            var artwork = [];
            if (track.pic) {
                artwork.push({ src: track.pic, sizes: '512x512', type: 'image/jpeg' });
            }
    ...
    function bindMediaSessionHandlers() {
        ...
        try {
            ...
            } catch (e2) { /* 部分浏览器不支持 seekto */ }
        } catch (e) { /* ignore */ }
    }

    // 仅暂停音频（供视频互斥 / Media Session 使用，不触发「起播」）
    function pauseOnly() {
        if (audio.paused && !state.isPlaying) return;
        audio.pause();
    ...
    function loadTrack(index, autoPlay) {
        ...
        try {
            localStorage.setItem('music-player-index', String(index));
        } catch (e) { /* ignore */ }
    ...
    function setPlayMode(mode) {
        state.playMode = mode;
        try {
            localStorage.setItem('music-player-mode', String(mode));
        } catch (e) { /* ignore */ }
    ...
    async function init() {
        ...
        try {
            ...
            if (state.playlist.length > 0) {
                ...
                // 硬刷新恢复曲目索引；无记录时随机模式才随机选一首。不自动 play()
                var startIndex = 0;
                var storedIndex = localStorage.getItem('music-player-index');
                if (storedIndex !== null) {
                    var idx = parseInt(storedIndex, 10);
    ...
    // 视频起播 → 暂停音乐（后启者优胜；pauseOnly 只发 isPlaying:false，不会反触发视频起播）
