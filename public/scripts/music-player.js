/* Firefly music player view — cached external script */
(function () {
  function bindWidget(widget) {
    if (!widget || widget.dataset.ffMusicBound === "1") return;
    var viewConfigStr = widget.getAttribute("data-view-config");
  ...
  function bindAll() {
    document.querySelectorAll(".music-player-widget[data-view-config]").forEach(bindWidget);
  }
  ...
  // 导航栏 + 侧栏各挂一次 src；第二次只补绑未初始化的 widget
  if (window.__fireflyMusicPlayerBound) {
    boot();
    return;
  }
  window.__fireflyMusicPlayerBound = true;
