...
- `geometry-data.js` — 自研几何 v2.1（ADR-0005 纯黑圆润）：形状族 5 个（正圆 blob / 鹅卵石 pebble / 卵形 egg / 圆角六边形 hex / 水滴 teardrop），25 组实心眼型（超椭圆图元按宏观参数重产，大号白竖椭圆族），纯黑 palette（`black.light/dark` 为线条色，`black.body` 为身体填充）。非 xAI 资产。
