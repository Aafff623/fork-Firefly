/* L2 — FireflyCharacter. Orchestrates pose, eyes, tricks, overlays. Source $_t + sd(). */
(function (g) {
  const M = g.FFLY_MATH;
  const T = g.FFLY_TABLES;
  const { applyPose, nextGaze } = g.FFLY_POSE;
  const TR = g.FFLY_TRICKS;
  const EY = g.FFLY_EYES;
  const FX = g.FFLY_FX;
  const {
  ...
  class FireflyCharacter {
    ...
    setShape(name) {
      if (!g.FFLY_GEO.shapes[name] || name === this.shapeName) return;
      const R = g.FFLY_GEO.Re;
      const k = K2(clamp(this.shapeSpring.x, 0, 1));
      const rest = FX.shapeMetrics(g.FFLY_GEO.shapes[this.shapeName], R);
  ...
  g.FireflyCharacter = FireflyCharacter;
  g.FFLY_META = {
    groups: GROUPS,
