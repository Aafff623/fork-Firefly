...
(function (global) {
  ...
  class OverlayLayer {
    ...
    extras(now, stateAt, cur, prev, yl, mix) {
      ...
      const wre = (A2 / (global.FFLY_GEO?.Re || Re0)) * iX;
  ...
  function turnAtOf(name, path, R) {
    ...
    const solid = global.FFLY_GEO.solids?.[name];
    if (!solid || !global.FFLY_MATH?.makeTurnAt) {
      turnAtCache.set(name, null);
      return null;
    }
    const fn = global.FFLY_MATH.makeTurnAt(solid, shapeRing(path, R), R);
