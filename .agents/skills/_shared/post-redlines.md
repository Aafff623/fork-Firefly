...
| 封面 | `image:` / 列表卡背景必须使用已存在的合规本地素材或 R2 图片；不在发布流程中调用模型生图。正文插图可用园主截图、官方图或合规检索素材；缺图标记待补。点名本站角色先使用 `SpritePet` / `maid-deepseek-whale` 的真实素材。 |
...
| 来源 | 默认 |
|---|---|
| 合集（早报 / 热榜） | 只使用官方 OG / README / 公告图等已有合规素材 → R2；缺图标记待补 |
| 公众号 | 先 `source/images/` 精选；禁止只留微信外链 |
| 会话 / 调研成帖 | **封面**使用已有合规素材；**正文**可用园主截图 / 官方 → 合规网图；缺图标记待补 |
