...
4 风格化 → 按 humanizer-output-style 校准语气；标题像人起的；去 AI 味
...
6. **别写成操作报告**：这是给人读的知识/文章，不是给工具看的交接单。动词是你的观点（"我建议""这里坑在"），不是你的动作（"我执行了""我调用"）。
...
## 配图：优先 MiniMax 生图，别靠 Mermaid 凑

**生图优先级高于 Mermaid。** 能用一张好图说清的事，就不画 Mermaid；Mermaid 只在「流程 / 时序」这类必须用图示的场景保留，且标签写人话。
...
- 调用 `humanizer-output-style` 校准：中文为主、工程师口吻、直接有判断、不装不捧。
