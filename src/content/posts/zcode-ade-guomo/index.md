...
updated: 2026-08-15T18:55:00
description: 8 月 13 日晚上看到 V4 Pro 和 GLM-5.3 对狙之后，我把 ZCode 下下来，把 Harness 五块、配置目录和宿主 API 与 MCP 的分路摊开。
image: ./cover.jpg
tags: [ZCode, GLM, ADE, Harness, MCP]
category: 指南
collections: [tool-zcode]
draft: false
lang: ''
...
直到 <span class="metric metric-ver">8/13</span> 那天晚上🌙，我发现 DS 官方在 API 平台悄摸摸把 **V4 Pro** 正式版挂了出来。本来只是想探探，没想到智谱 **GLM-5.3** 直接中门对狙。

> 突然想起之前在咸鱼🐟️捡漏的 GLM 老 Lite，再加上智谱官方 Harness 工具那 <span class="metric metric-high">150%</span> 额度和对 GLM 系列高得吓人的缓存命中，我干脆先把 **ZCode** 下下来，看看这位*没落的国模一哥*现在到底什么成色。

于是就有了后面这些。

它帮我完成了两个开源项目（**Velaterm** 和 **Claude-hud**）交叉情况下的顽固 bug。
虽然是纯文本模型，但它跑了 <span class="metric metric-high">50</span> 多分钟（将近一个小时），修复过程非常完整，To-Do List 也列得清清楚楚。配合识图的 **MiniMax MCP**、官方的图片理解，再加上 **Chrome MCP** 和 **Playwright**，它把长任务直接打通，还把交叉和回归测试一起做完了。
...
![ZCode 工作台：任务、Skills 和中间输入框](./images/fig-01-zcode-ui.webp)

也就是说，它是一款对标目前主流 ADE 的国内落地化产品。
对智谱 **GLM** 系列做了深度的缓存优化和适配，还提供了 <span class="metric metric-high">150%</span> 的额度供应。

比较美中不足的是，GLM 和 DS 系列目前都是*纯文本* model，识图得额外接 vision 模型。
...
![Agent、Skills、MCP 和 Subagent 怎么围在一起](./images/fig-02-agent-ecology.png)
...
hub 里目录一多（我这边曾经到过 <span class="metric metric-low">121</span> 个），每轮先注入的是 name + description。
...
![MCP 从宿主经 Client、Protocol 接到外部工具](./images/fig-03-mcp-arch.png)
...
flowchart LR
    ...
    subgraph API["模型通道"]
        C["宿主 API<br/>直连官方 / 自定义 Provider"]
        F["模型推理<br/>GLM / 第三方"]
    end
    subgraph MCP["工具通道"]
        D["MCP 子进程<br/>npx / uvx"]
        E["外部工具<br/>浏览器 / 搜索 / 视觉"]
    end
    A --> B
    B --> C
    ...
    class C,F api
    class D,E mcp
    classDef user fill:#F4C27A,stroke:#B45309,stroke-width:2px,color:#1C1917
    classDef host fill:#7DD3FC,stroke:#0369A1,stroke-width:2px,color:#0C4A6E
    classDef api fill:#6EE7B7,stroke:#047857,stroke-width:2px,color:#064E3B
    classDef mcp fill:#F9A8D4,stroke:#BE185D,stroke-width:2px,color:#831843
```
...
![长任务能力随模型发布年份往上走](./images/fig-04-long-task.png)
