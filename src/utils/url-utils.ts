...
export function getTagUrl(tag: string): string {
	if (!tag) return url("/timeline/");
	return url(`/timeline/?tag=${encodeURIComponent(tag.trim())}`);
}
...
export function getCategoryUrl(category: string | null): string {
	if (
		...
		return url("/timeline/?uncategorized=true");
	return url(`/timeline/?category=${encodeURIComponent(category.trim())}`);
}
